...
const CheckoutPage = (props) => {
    ...
    useEffect(()=>{
        const getCartItems = async ()=>{
            try{
                const response = await backend.get('/api/get_cart_items',{params:{userId:localStorage.getItem('userId')}});
        ...
        <div className="checkoutpage">
            <Accordion defaultActiveKey="0" style={{margin:"30px 10px"}}>
                <Card style={{width:"800px"}}>
                    <Accordion.Toggle as={Card.Header} eventKey="0">Select a delivery address</Accordion.Toggle>
                ...
                <Card style={{width:"800px"}}>
                    <Accordion.Toggle as={Card.Header} eventKey="1">Add a new address</Accordion.Toggle>
                ...
                <Card style={{width:"800px"}}>
                    <Accordion.Toggle as={Card.Header} eventKey="2">Select Payment method</Accordion.Toggle>
...
const AddressList = ({selectedAddress,setSelectedAddress})=>{
    ...
    const addresses = addressList.map((item,index) =>{
       return <AddressCard data={item} setUpdater={setUpdater} key={index} selectedAddress={selectedAddress} setSelectedAddress={setSelectedAddress}/>
    })
